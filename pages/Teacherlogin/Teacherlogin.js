...
Page({
  data: {
   ...
   onLoad:function(options){
    //  从缓存获取账号密码
    let tc_account=wx.getStorageSync('tc_account');
    let tc_pwd=wx.getStorageSync('tc_pwd');
   //  当密码不是第一次输入的时候自动填上账号密码


if(tc_account && tc_pwd){
 this.setData({
  account: tc_account,
  password : tc_pwd
 })
...
//注册功能的实现
    wx.request({
      ...
      url:'https://hemantower.com/public/index.php/api/v1/TeacherLogin',
      data: {
...
//如果返回值的code为201说明登陆成功
        if (res.data.code == 201) {
          wx.setStorageSync('tc_account', account);
          wx.setStorageSync('tc_pwd', password);
